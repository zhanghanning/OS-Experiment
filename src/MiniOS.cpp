...
void updateNextIndices(std::vector<PCB>& processes, const std::vector<int>& readyQueue) {
    for (auto& process : processes) {
        process.nextIndex = -1;
    }

    for (std::size_t i = 0; i < readyQueue.size(); ++i) {
        int current = readyQueue[i];
        int next = readyQueue[(i + 1) % readyQueue.size()];
        processes[current].nextIndex = next;
    }
}
...
void MiniOS::runMemoryExperiment(bool useDefaultInput) {
    ...
    int frameCountForDetail = useDefaultInput
        ? 4
        : readInt("请输入用于详细展示的物理块数 [默认 4，范围 4-32]：", 4, 4, 32);
...
void MiniOS::runIntegratedExperiment(bool useDefaultInput) {
    ...
    int frameCount = useDefaultInput
        ? 4
        : readInt("请输入综合运行的物理块数 [默认 4，范围 4-32]：", 4, 4, 32);
    unsigned int seed = static_cast<unsigned int>(useDefaultInput
        ? 2026
        : readInt("请输入随机种子 [默认 2026]：", 2026, 1, 999999));
